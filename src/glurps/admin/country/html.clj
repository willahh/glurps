...
(defn list-html 
  ...
  (main/admin-page-html-wrapper 
   ...
   (crud-list/get-html-wrapper session
                               ...
                               state
                               setting/list-conf
                               (country-dao/enable-count)
                               country-dao/get-list
                               (country-dao/disable-count)
                               list-action-html-fn)))

(defn show-html 
  [session params state]
  (let [id (:id params)
        record (country-dao/find-by-id id)
...
(defn update-html
  ...
  (let [id-list (clojure.string/split (:id params) #"-")
        records (let [records (map (fn [id]
                                     (country-dao/find-by-id id)) id-list)]
                  records)]
    (main/admin-page-html-wrapper
     session
     params
